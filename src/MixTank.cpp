...
void MixTank::toggle_mischer_ventil() {
    mischer_ventil = !mischer_ventil;
    if (mischer_ventil) {
        std::thread t_mischer(&MixTank::mischer_ventil_open_loop, this);
        t_mischer.detach();
    }
}

void MixTank::toggle_lager_ventil() {
    lager_ventil = !lager_ventil;
    if (lager_ventil) {
        std::thread t_lager(&MixTank::lager_ventil_open_loop, this);
        t_lager.detach();
    }
}

void MixTank::mischer_ventil_open_loop() {
    while (mischer_ventil && (fuellstand > 0)) {
        ...
        socketHandler->sendMessage("level_mixtank " + std::to_string(fuellstand));
        Sleep(100);
    }
}

void MixTank::lager_ventil_open_loop() {
